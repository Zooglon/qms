...
async function sendSupplierEmail(supplier) {
  ...
  console.log("Not yet implemented, sending email to", supplier.email);
}

const isValid = (i) =>
  /^[A-Z]{1,2}[0-9]{1,2}[A-Z]{0,1} ?[0-9][A-Z]{2}$/i.test(i.replace(/\s/g, ""));
...
export const invoke = async ({ payload }) => {
  ...
  let pc = sa.match(/[A-Z]{1,2}[0-9]{1,2}[A-Z]{0,1} ?[0-9][A-Z]{2}/g);
  console.log("PC1", sa, pc);
  pc = pc[0];
  console.log("PC", pc);
  ...
  for (let i = 0; i <= qn; i++) {
    const sn = ssl[i].supplierName;
    try {
      sendSupplierEmail(ssl[i]);
    } catch (error) {
